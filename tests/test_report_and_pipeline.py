"""End-to-end tests for the offline pipeline, report generation and eval checks."""
...
def test_format_metric():
    ...
    assert format_metric("market_cap", 3.45e12) == "3.45T USD"
    assert format_metric("free_cashflow", 1.08e11) == "108.00B USD"
...
def test_metrics_table_contains_labels(aapl_bundle):
    ...
    assert "| P/E (trailing) | 34.70 |" in table
    assert "| Net margin | 26.3 % |" in table
...
def test_template_pipeline_and_report(aapl_bundle, tmp_path):
    ...
    assert set(result.sections) == {
        "fundamental_analysis", "qualitative_assessment", "bull_bear", "memo"
    }

    markdown = render_markdown(aapl_bundle, result)
    for section in REQUIRED_SECTIONS:
        assert section in markdown
    assert "NOT constitute investment advice" in markdown
...
def test_eval_checks_detect_problems(aapl_bundle):
    ...
    # a missing section is detected
    markdown = render_markdown(aapl_bundle, result).replace(
        "## Fundamental Analysis", "## Broken")
    ok, missing = check_sections(markdown)
    assert not ok and "## Fundamental Analysis" in missing

    # a hallucinated number is detected
    result.sections["memo"] += " The P/E ratio stands at 999.99."
    halluc = check_hallucinations(result, aapl_bundle)
